...
router.post('/request', async (req, res) => {
    try {
        ...
        const data   = { friendRequestList: [...new Set([...recipientUser.friendRequestList, senderUser._id])] }
        const myData = { myFriendRequestList: [...new Set([...senderUser.myFriendRequestList, recipientUser._id])] }
...
router.post('/accept', async (req, res) => {
    try {
        ...
        const dataRecipient = { friends: [...new Set([...recipientUser.friends, adoptedUser._id])],
                                myFriendRequestList: recipientUser.myFriendRequestList.filter(friend => friend.toString() !== adoptedUser._id.toString()) }
...
router.post('/request/delete', async (req, res) => {
    try{
        ...
        if (typeRequestFriend === 'other') {
            ...
            const data    = user.friendRequestList.filter(friend => friend.toString() !== userStranger._id.toString())
            const result  = { friendRequestList: data }

            const senderData   = userStranger.myFriendRequestList.filter(friend => friend.toString() !== user._id.toString())
            ...
            const data    = user.myFriendRequestList.filter(friend => friend.toString() !== userStranger._id.toString())
            const result  = { myFriendRequestList: data }
    
            const dataStranger      = userStranger.friendRequestList.filter(friend => friend.toString() !== user._id.toString())
