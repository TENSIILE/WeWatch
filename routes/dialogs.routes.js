...
router.post('/create', auth, async (req, res) => {
  try {
    ...
    if (!friendFinded)
      return res
        .status(403)
        .json({ message: 'Данный пользователь не состоит у Вас в друзьях!' })

    const isExistsDialogAuthor = await Dialog.find({
      author: req.user.userId,
      partner,
    })
    const isExistsDialogPartner = await Dialog.find({
      author: partner,
    ...
    if (!!isExistsDialogAuthor.length || !!isExistsDialogPartner.length) {
      return res
        .status(208)
        .json({
          message: 'Данный диалог уже существует!',
          dialogId: isExistsDialogAuthor[0]._id,
        })
    }
