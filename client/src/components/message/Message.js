...
export const Message = ({ 
    ...
    const [timeMessage, setTimeMessage]  = useState(distanceInWordsToNowWrapper(objMessage.create_at))
    ...
    useEffect(() => {
        const wrap = async () => {
            if (isMe) {
                setFullname(infoUser.userAdditional.name + ' ' + infoUser.userAdditional.lastname)
                setAvatar(infoUser.userAdditional.avatar)
                
            } else {
                setFullname(currentDialog.userAdditional.name + ' ' + currentDialog.userAdditional.lastname)
    ...
    const messageTextJSX = () => {
        return reactStringReplace(parserLinks(textMessage), /:(.+?):/g, (match, i) => (
            <Emoji 
