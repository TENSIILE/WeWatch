...
export const CatalogList = ({label, value, onChange, text, setText, list, setList}) => {
    const alert = useContext(ContextAlert)

    const addNewLanguageToList = e => {
        const isNoneEmpty = list.map(item => item.toLowerCase()).includes(text.toLowerCase())

        if (e.key === 'Enter' && text.trim() !== '' && list) {

            if (isNoneEmpty) return alert.show('warning','Нельзя добавить несколько одинаковых языков!', 'Предупреждение!')

            setList(list.map(item => firstLetterToUpperCase(item)))
            setText.setChangeDataProfile({...setText.changeDataProfile, 'Языки:': ''})
            setList([...list, text])
        }
    }
    ...
    return (
        <div className='catalog_list'>
            ...
            <ReactSVG 
                src={info} 
                onClick={() => alert.show('info', 'Для того, чтобы добавить новый язык, нажмите клавишу Enter', 'Информация!')}
            />
            <ul className={`list-item ${!!list.length ? 'margin-on' : null}`}>
                {
                    list.map((lang, index) => {
                        return (
