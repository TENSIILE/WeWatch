...
export const SliderHorizontal = ({ 
    ...
    const swiperHandler = side => {
        switch (side) {
            case 'left':
                ...
                break
            default:
                if (position < (sliderRef.current.scrollWidth - sliderRef.current.offsetLeft) - sliderRef.current.offsetWidth) 
                    setPosition(position + offset)
                break
        }
    }
    ...
    const resize = debounce(() => {
        if (sliderRef.current && sliderRef.current.offsetWidth < sliderRef.current.scrollWidth) {
            setVisibleBtn(true)
    ...
    return (
        ...
        >
            <div className={`slider ${newClass}`}>
                ...
                {
                    isVisibleButton && (
                        <div className={classnames('btn-slider-container', {'visible': visibleBtn})}>
                            <button
