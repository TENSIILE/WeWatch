...
export const ActionMyFriends = ({ fullDataFriend }) => {
    ...
    const russianNames    = new RussianName(`${fullDataFriend.name} ${fullDataFriend.lastname}`)
    ...
    const modalActive = () => {
        ...
        modal.changeTitle(DELETE_FRIEND, <>Вы действительно хотите удалить <strong className='text-highlight'>{nameLastNameRod}</strong> из друзей?</>)
    ...
    const startСommunication = useCallback(async () => {
        main.infoUserDialogs.forEach(dialog => {
            if ((dialog.author === auth.userId && dialog.partner === fullDataFriend.user) || (dialog.author === fullDataFriend.user && dialog.partner === auth.userId)) {
        ...
        if (!isExistsDialog.current) {
            const { dialogId } = await request(`${config.hostServer}/api/dialogs/create`, 'POST', { partner: fullDataFriend.user }, {
    ...
    return (
        <>
            <ul>
                <li 
                    onClick={viewProfileModalActive}
                >
                    Профиль
                </li>
                <li 
