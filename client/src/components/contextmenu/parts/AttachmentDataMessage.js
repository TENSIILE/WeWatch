...
export const AttachmentDataMessage = () => {
  ...
  const onUploadedFiles = type => {
    ...
    switch (type) {
      case 'image':
        logicChat.setImagesUpload([
          ...logicChat.imagesUpload,
          ...images.current.files,
        ])
        break
      case 'video':
        logicChat.setVideosUpload([
          ...logicChat.videosUpload,
          ...videos.current.files,
        ])
        break
      case 'audio':
        logicChat.setAudiosUpload([
          ...logicChat.audiosUpload,
          ...audios.current.files,
        ])
        break
      default:
        logicChat.setDocumentsUpload([
          ...logicChat.documentsUpload,
          ...documents.current.files,
        ])
        break
    }
  }
      ...
      <ul>
        ...
        <li>
          <label htmlFor='documents'>
            Файлы
            {!!logicChat.documentsUpload.length && (
              <span className='badge-counter'>
                {logicChat.documentsUpload.length}
              </span>
            )}
