...
export const ImageDirectory = ({
  ...
  const onSelect = (e, img) => {
    const link =
      e &&
      e.target
        .closest('.image-directory__photo-container')
        .querySelector(`.image-directory__set-background[data-url='${img}']`)
  ...
  const init = () => {
    const background = reference.current.querySelectorAll(
      `.image-directory__set-background`
    )
  ...
  return (
    <div className='image-directory'>
      <div className='image-directory__list-photos' ref={reference}>
        {arrayImages.map((image, i) => {
          return (
            ...
            >
              <div className='image-directory__set-background' data-url={image}>
                <ReactSVG src={check} className='image-directory__icon' />
              </div>
              <img className='image-directory__photo' src={image} alt='' />
            </div>
          )
        })}
      </div>
      <div className='image-directory__uncover' onClick={onToggle} />
    </div>
  )
}
