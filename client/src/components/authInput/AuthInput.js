...
export const AuthInput = ({
  ...
  const checkedLoginValid = async () => {
    if (text.trim().toString()) {
      try {
        await request(
          `${config.hostServer}/api/auth/login/checked_valid`,
          'POST',
          { login: text }
        )
      } catch (e) {
    ...
    <div className='authInput' style={style}>
      ...
      {type !== 'password' ? (
        <>
          ...
          {type === 'email' && (
            <>
              <ReactSVG src={checked} className='validate-icon good' />
              <ReactSVG src={close} className='validate-icon bad' />
            </>
          )}
          {icon && (
            <ReactSVG src={icon} onClick={onClickIcon} className='icon-input' />
          )}
        </>
      ) : (
        <>
