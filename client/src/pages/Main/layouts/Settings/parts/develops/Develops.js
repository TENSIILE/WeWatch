...
export const Develops = () => {
    ...
    const onAsyncGetTokenUser = async () => {
        if (!inputRef.current.value) {
            try {
                const { tokenApi } = await request(`${config.hostServer}/api/getInfo/user/getTokenApi?userId=${auth.userId}`, 'GET', null, {
    ...
    return (
        <div className="develops">
             <ul className='clauses-tining'>
                ...
                <li 
                    className={classnames('clause transition overflow-h', {'hidden': !settings.switchBtn[DEVELOPER_MODE]})} 
                    ref={heightDev.ref}
                >
                    <div className="tuning-control">
                        <div className="text-info-detail">
                            ...
                            <span className='description'>Ваш токен будет нужен для индентификации ваших действий через WeWatch API, поэтому, пожалуйста, держите Ваш токен подальше от чужих глаз!</span>
                        </div>
