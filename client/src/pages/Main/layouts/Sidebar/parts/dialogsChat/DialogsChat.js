...
export const DialogsChat = () => {
  const { infoUserDialogs, isReloading, setInfoUserDialogs } = useContext(
    ContextMain
  )
  ...
  useEffect(() => {
    ...
    setInfoUserDialogs(
      infoUserDialogs.sort((a, b) => {
        if (!!a.messages.length && !!b.messages.length) {
          return (
            new Date(b.messages[b.messages.length - 1].create_at) -
            new Date(a.messages[a.messages.length - 1].create_at)
          )
        }
      })
    )
                    ...
                    return (
                      <Dialog
                        ...
                        isMe={
                          ...
                          auth.userId ===
                            dialog.messages[dialog.messages.length - 1].author
                        }
                        dialogData={dialog}
                      />
                    )
