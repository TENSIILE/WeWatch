...
export const LogicCreatingRoom = ({ children }) => {
  ...
  const { listRequestFriends, rerender, setRerender } = useContext(
    ContextGetInfo
  )
  ...
  const appendToListAllLinksToVideos = e => {
    if (e.key === 'Enter') {
      try {
        const finded = listAllLinksToVideos.find(
          ({ link }) => link === valuesInputs.linksToAllVideo
        )
  ...
  const deleteFromListAllLinksToVideos = link_video => {
    setListAllLinkToVideos(
      listAllLinksToVideos.filter(link => link.link !== link_video)
    )
  }
  ...
  const acceptSecurityKey = () => {
    if (!!valuesInputs.textSecurityKey) {
      ...
      alert.show(
        'warning',
        'Вы забыли установить пароль для вашей будущей комнаты!',
        'Ошибка!'
      )
    }
  }
  ...
  const changeCheckboxReadyLinks = () =>
    setCheckboxReadyLinks(!checkboxReadyLinks)
  ...
  const filterAddedFriendInRoomContextmenu = () => {
    return !!listRequestFriends
      ? listRequestFriends.userFriends.filter(
          friend => !listAddedFriends.includes(friend)
        )
      : []
  }
  ...
  const createRoomHandler = async () => {
    try {
      ...
      if (checkboxReadyLinks) {
        listAllLinksToVideosFormat = listAllLinksToVideos.map(
          video => video.link
        )
      }
      ...
      const data = await request(
        `${config.hostServer}/api/room/create`,
        'POST',
        dataRoom,
        {
          authorization: `Bearer ${auth.token}`,
