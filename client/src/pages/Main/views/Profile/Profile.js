...
export const Profile = () => {
  ...
  const saveDataProfile = async () => {
    try {
      ...
      await request(
        `${config.hostServer}/api/getInfo/user/savedata`,
        'POST',
        { userId: auth.userId, userInfo: readyData },
  ...
  useEffect(() => {
    if (
      !!infoUser &&
      infoUser.userAdditional.languages.toString() !== 'Не указано'
    ) {
    ...
    <>
      ...
      <div className='user-account beautiful-scrollbar mini'>
        <div className='user-header'>
          <Image
            src={!!infoUser ? infoUser.userAdditional.header : header}
            id='header'
          />
          <ButtonMini
            icon={pictures}
            ...
            <div className='avatar'>
              ...
              >
                <input
                  ...
                  id='file'
                  onChange={() =>
                    main.onSendImageProfileAsync('avatar', refAvatar)
                  }
                  accept='image/jpeg,image/png,image/gif'
                  ref={refAvatar}
                />
            ...
            <div className='user-fullname'>
              <div className='username'>
                {!!infoUser && (
                  <>
                    ...
                    <p>
                      {infoUser.userAdditional.name +
                        ' ' +
                        infoUser.userAdditional.lastname}
                    </p>
                  </>
                )}
              </div>
              <p
                ...
                className={classnames({
                  'edit-status': changeDataProfile['Статус:'] && changeProfile,
                })}
              >
                {changeDataProfile['Статус:']
                  ? changeDataProfile['Статус:']
                  : !!infoUser && infoUser.userAdditional.status}
              </p>
            </div>
          </div>
              ...
              changeProfile ? (
                ...
                <>
                  {infoUser.tableInformation.map((row, index) => {
                    return <RowInfo key={index} label={row[0]} value={row[1]} />
                  })}
                </>
              )
            ) : (
