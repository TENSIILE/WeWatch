...
export const RestorePass = () => {
    ...
    const sendCodeToEmail = async () => {
        try {
            await request(`${config.hostServer}/api/recovery/getKeyByMail`, 'POST', { email: form.remember_password_email })
            setCodeEntryInputRecovery(true)
            alert.show('success', `На почту ${form.remember_password_email} был выстан код подтверждения!`, 'Успешно!')
        } catch (e) {
    ...
    const confirmCode = async () => {
        if (form.codeCheck === '') {
            ...
            try {
                await request(`${config.hostServer}/api/recovery/keyVerification`, 'POST', { email: form.remember_password_email, key: form.codeCheck })
                ...
                <>
                    <div className='code_container' style={{ display:'flex', alignItems:'center', width:'360px', justifyContent:'center'}}>
                        ...
                        <Button text='ОК'
                            classNames='btn simple'
                            onClick={confirmCode}
                            style={{padding:0, width:'20%', zIndex:10, marginTop:23, marginLeft:'1em'}}
