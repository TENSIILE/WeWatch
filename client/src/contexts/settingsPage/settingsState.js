...
export const SettingsState = ({ children }) => {
    ...
    const [checkDecideDeleteAccount, setCheckDecideDeleteAccount] = useState(false)

    // Конфиденциальность
    
    const [addingToFriend, setAddingToFriend] = useLocalStorage(ADDING_TO_FRIEND, {
        'all':true,
        'nobody':false,
    })
    ...
    const [selectImageBackgroundChat, setSelectImageBackgroundChat] = useLocalStorage(SELECT_IMAGE_BACKGROUND_CHAT, '')

    const [backgroundChatOpen, setBackgroundChatOpen]               = useLocalStorage(IS_BACKGROUND_CHAT_OPEN, false)

    const [selectedTheme, setSelectedTheme]                         = useLocalStorage(SELECTED_THEME, 'blue-theme')

    const [cubes, setCubes] = useLocalStorage(CUBES, [
        {id: v4(), colors: ['#ED5563','#C73D4A','#DC3545'], turnOn: false, text:'Красная', textColor:'#DC3545', nameClassName:'red-theme'},
        {id: v4(), colors: ['#72EB87','#42CB59','#53D769'], turnOn: false, text:'Зеленая', textColor:'#53D769', nameClassName:'green-theme'},
        {id: v4(), colors: ['#47A0FF','#0074F0','#007BFF'], turnOn: true,  text:'Синяя', textColor:'#007BFF', nameClassName:'blue-theme'},
        {id: v4(), colors: ['#FFD24A','#EBB000','#FFC107'], turnOn: false, text:'Желтая', textColor:'#FFC107', nameClassName:'yellow-theme'},
        {id: v4(), colors: ['#FF9D42','#F37500','#eb8628'], turnOn: false, text:'Оранжевая', textColor:'#eb8628', nameClassName:'orange-theme'},
        {id: v4(), colors: ['#2EB8CE','#1396AB','#17A2B8'], turnOn: false, text:'Голубая', textColor:'#17A2B8', nameClassName:'sky-theme'},
        {id: v4(), colors: ['#B778D1','#9244B1','#9B59B6'], turnOn: false, text:'Фиолетовая', textColor:'#9B59B6', nameClassName:'purple-theme'},
    ])
    ...
    const [successfulIdentification, setSuccessfulIdentification] = useState(false)

    // Дисплей

    const [displayBrightness, setDisplayBrightness] = useLocalStorage(DISPLAY_BRIGHTNESS, 100)

    // Микрофон и камера

    const [microphoneAndCamera, setMicrophoneAndCamera] = useLocalStorage(MICROPHONE_AND_WEBCAMERA, {
        'microphone':{
            'volumes': {
                'volume-input': 10,
                'volume-output': 10
    ...
    const [speedRangeSliderImage, setSpeedRangeSliderImage] = useLocalStorage(SPEED_RANGE_SLIDER_IMAGE, 200)

    // Методы

    const onChangeVolume = () => {
        if (previewWebcamera.current) {
            previewWebcamera.current.volume = microphoneAndCamera.microphone.volumes['volume-input'] / 100
        }
    }
    
    const usingDevices = stream => {
        if (!isPreviewWebcamera && previewWebcamera.current) return previewWebcamera.current.srcObject = null
    ...
    const onChooseVolumeDevices = (device, value) => {
        setMicrophoneAndCamera({...microphoneAndCamera, 'microphone': {
    ...
    const onChooseMicrophoneAndWebcamera = device => {
        switch (device.kind) {
            case 'audiooutput':
                setMicrophoneAndCamera({...microphoneAndCamera, 'microphone': {
                    'volumes': {...microphoneAndCamera.microphone.volumes}, 'devices': {
                        ...microphoneAndCamera.microphone.devices, 'device-output': device
                    }
                }})
                break
            case 'audioinput':
                setMicrophoneAndCamera({...microphoneAndCamera, 'microphone': {
                    'volumes': {...microphoneAndCamera.microphone.volumes}, 'devices': {
                        ...microphoneAndCamera.microphone.devices, 'device-input': device
                    }
                }})
                break
            case 'videoinput':
                setMicrophoneAndCamera({...microphoneAndCamera, 'webcamera': {
                    ...microphoneAndCamera.webcamera, 'device': device
                }})
                break
            default:break
        }
    ...
    const onSetMicroAndCameraByDefault = (arrayDevices) => {
        ...
        if (!Object.entries(microphoneAndCamera.microphone.devices["device-input"]).length) { 
            const findedDevice = arrayDevices.find(device => device.deviceId === 'default' && device.kind === listKinds[0])
            onChooseMicrophoneAndWebcamera(findedDevice)   
        }

        if (!Object.entries(microphoneAndCamera.microphone.devices["device-output"]).length) {
            const findedDevice = arrayDevices.find(device => device.deviceId === 'default' && device.kind === listKinds[1])
            onChooseMicrophoneAndWebcamera(findedDevice)   
        }

        if (!Object.entries(microphoneAndCamera.webcamera.device).length) {
            const findedDevice = arrayDevices.find(device => device.kind === listKinds[2])
            onChooseMicrophoneAndWebcamera(findedDevice)
        }
    }
    ...
    const changePasswordUser = async () => {
        try {
            const { message } = await request(`${config.hostServer}/api/recovery/assigningNewPasswordFromAccount`, 'POST', { new_password: passwordInput }, {
    ...
    const changeSwitchBtn = (nameSwitch, value = NaN) => {
        setSwitchBtn({ ...switchBtn, [nameSwitch]: !isNaN(value) ? value : !switchBtn[nameSwitch] })  
    }
    ...
    const onToggleImageDirectory = action => {
        const top = imageDirectoryRef.current.lastElementChild.getBoundingClientRect().top
    ...
    const onSaveSettings = async () => {
        const settings = {
            ...
            [SELECT_IMAGE_BACKGROUND_CHAT]: JSON.parse(localStorage.getItem(SELECT_IMAGE_BACKGROUND_CHAT)),
            [IS_BACKGROUND_CHAT_OPEN]: JSON.parse(localStorage.getItem(IS_BACKGROUND_CHAT_OPEN)),
            [SELECTED_THEME]: JSON.parse(localStorage.getItem(SELECTED_THEME)),
            [CUBES]: JSON.parse(localStorage.getItem(CUBES)),
            [DISPLAY_BRIGHTNESS]: JSON.parse(localStorage.getItem(DISPLAY_BRIGHTNESS)),
            [MICROPHONE_AND_WEBCAMERA]: JSON.parse(localStorage.getItem(MICROPHONE_AND_WEBCAMERA)),
            [LIST_DEVICES]: JSON.parse(localStorage.getItem(LIST_DEVICES)),
            [OVERLAY]: JSON.parse(localStorage.getItem(OVERLAY)),
            [SPEED_RANGE_SLIDER_IMAGE]: JSON.parse(localStorage.getItem(SPEED_RANGE_SLIDER_IMAGE))
        }
        ...
        if (!localStorage.getItem(config.lastVersionSettingsFromServer)) {
            ...
            const data = await (await fetch(`${config.hostServer}/api/settings/get?userId=${userId}`, option)).json()
            localStorage.setItem(config.lastVersionSettingsFromServer, data.settings)
        }
    
        if (localStorage.getItem(config.lastVersionSettingsFromServer) === settingsStringifyJSON) return
        
        await request(`${config.hostServer}/api/settings/save`, 'POST', {settings}, {
            authorization: `Bearer ${token}`
        })
        localStorage.setItem(config.lastVersionSettingsFromServer, settingsStringifyJSON)
    }
    ...
    useEffect(() => {
        ...
        const wrap = async () => {
            await navigator.mediaDevices.getUserMedia({audio: {
                deviceId: microphoneAndCamera.microphone.devices['device-input'].deviceId,
            ...
            devices.forEach(device => {
                let index = newDevices.findIndex(findDevice => {
                    return findDevice.groupId === device.groupId && findDevice.kind === device.kind
                })
    ...
    return (
        <ContextSettings.Provider value={{
            cubes, changeTheme, arrayImages, selectImageBackgroundChat, setSelectImageBackgroundChat,
            changeSwitchBtn, switchBtn, backgroundChatOpen, setBackgroundChatOpen, 
            imageDirectoryRef, onToggleImageDirectory, selectedTheme, addingToFriend,
            setAddingToFriend, speedRangeSliderImage, setSpeedRangeSliderImage,
            passwordInput, setPasswordInput, checkDecideDeleteAccount, 
            setCheckDecideDeleteAccount, changePasswordUser, onSaveSettings,
            overlay, setOverlay, listDevices, onChooseMicrophoneAndWebcamera,
            microphoneAndCamera, previewWebcamera, isPreviewWebcamera, setIsPreviewWebcamera,
            onChooseVolumeDevices, loading, inputTokenApi, setInputTokenApi,
            displayBrightness, setDisplayBrightness, successfulIdentification,
