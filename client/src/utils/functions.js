...
export const getUserToken = (func = () => {}) => {
    return new Promise(resolve => {
        let token = JSON.parse(localStorage.getItem(config.nameDataLocalStorage))?.token
...
export const isTodayDay = datetime => {
    return new Date().toLocaleDateString() === new Date(datetime).toLocaleDateString()
}

export const compareDates = (datetime1, datetime2) => {
    return new Date(datetime1).toLocaleDateString() === new Date(datetime2).toLocaleDateString()
} 

export const restrictOutsidePlaybackground = (num, step = 1, vector = 'back') => {
    switch (vector) {
        case 'back':
...
export const uploaderFiles = async (file, dialogId, messageId) => {
    ...
    await fetch(`${config.hostServer}/upload/dialogs?dialogId=${dialogId}&messageId=${messageId}`, options)
}

export const matrix = () => {
    const pattern = ['q','w','e','r','t','y','u','i','o','p','a','s','d','f','g','h','j','k','l','z','x','c','v','b','n','m','1','2','3','4','5','6','7','8','9','0','-','=','!','@','#','$','%','^','&','*','(',')','_','+','`','.',',','>','<','?','"',':',';','|']
    let result = ''
