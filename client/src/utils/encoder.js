...
export class Racker {
  ...
  async encode(string_encode) {
    return new Promise(resolve => {
      ...
      const swipe_case_encoded = Racker.moveLowToUpAndUpToLowCases(
        base64.encode(reverse_enc_w_salt)
      )
      ...
      const final_minimaze_encode = Racker.checkAndRemoveEqually(
        Racker.moveLowToUpAndUpToLowCases(base64.encode(reverse_convert_to_hex))
      )
  ...
  async decode(string_decode) {
    return new Promise(resolve => {
      const split_string = Racker.splitByDigit(
        base64.decode(Racker.moveLowToUpAndUpToLowCases(string_decode)),
        2
      )

      const reverse_back_convert_to_int = Racker.reverseIntExpectNumWithZeros(
        split_string
      )

      const convert_to_int = reverse_back_convert_to_int.map(num =>
        parseInt(num, 35)
      )

      const swipe_case_decoded = Racker.moveLowToUpAndUpToLowCases(
        UTF8.getStringFromBytes(convert_to_int)
      )

      console.log('DECODE: ', convert_to_int, swipe_case_decoded)

      const reverse_decoded_with_salt = Racker.reverseString(
        base64.decode(swipe_case_decoded)
      )
